  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError('Tax data is not available for this country.');
      }
    } catch {
  ...
  return (
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
        <h2 className="mb-6 text-lg font-bold">Enter Your Details</h2>
      ...
      {/* Results */}
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
        <h2 className="mb-6 text-lg font-bold">Results</h2>

        {result ? (
          <div className="space-y-4">
            {selectedCountry && (
              <p className="text-sm text-slate-400">
                Net salary estimate for{' '}
            ...
            <div className="flex items-center justify-between border-b border-white/5 pb-3">
              <span className="text-sm text-slate-400">Gross Annual</span>
            ...
            <div className="mt-4">
              ...
              <div className="mt-1 flex justify-between text-xs">
                <span className="text-emerald-400">
                  Take Home ({((result.netAnnual / result.gross) * 100).toFixed(1)}%)
                </span>
                <span className="text-red-400">
                  Tax ({((result.totalTax / result.gross) * 100).toFixed(1)}%)
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex h-48 items-center justify-center text-slate-500">
            <p className="text-center text-sm">
              Enter your salary and select a country to see your estimated
              take-home pay.
            </p>
          </div>
        )}
