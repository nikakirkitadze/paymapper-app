...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: t('countriesTitle', { appName: APP_NAME }),
...
export default async function CountriesPage({
  ...
  const sortOptions = [
    { value: 'name', label: tc('sortName') },
    { value: 'salary', label: tc('sortSalary') },
    { value: 'cost', label: tc('sortCost') },
  ];

  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <AnimatedSection>
        <h1 className="mb-2 text-3xl font-extrabold">{tc('title')}</h1>
        <p className="mb-8 text-slate-400">
          {tc('subtitle', { count: countries.length })}
        </p>
      </AnimatedSection>
      ...
      {countries.length > 0 ? (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex items-center gap-3">
                    <span className="text-sm text-slate-400">
                      {tCommon('avgPrefix')} {formatCurrency(country.avgSalary, true)}{tCommon('perYear')}
                    </span>
                    {country.costIndex > 0 && (
                      <span className="rounded-full bg-white/5 px-2 py-0.5 text-xs text-slate-500">
                        {tCommon('colPrefix')} {country.costIndex.toFixed(0)}
                      </span>
                    )}
                  </div>
                </div>
        ...
        <div className="py-16 text-center">
          <p className="text-xl text-slate-400">{tc('noCountriesFound')}</p>
        </div>
      )}
    </div>
  );
