...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: tm('jobsTitle', { appName: APP_NAME }),
    description: tm('jobsDescription'),
...
export default async function JobsPage({ searchParams }: JobsPageProps) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      {/* Result count */}
      <p className="mb-4 text-sm text-slate-500">{tc('showingJobs', { count: jobs.length })}</p>
      ...
      {jobs.length > 0 ? (
            ...
            <AnimatedSection key={job.id} delay={Math.min(i * 0.03, 0.3)}>
              ...
              >
                ...
                <p className="mt-1 text-sm text-slate-400">
                  {job.salaryMin > 0
                    ? `${formatCurrency(job.salaryMin, true)} - ${formatCurrency(job.salaryMax, true)}${tc('perYear')}`
                    : tc('salaryDataComingSoon')}
                </p>
        ...
        <div className="py-16 text-center">
          <p className="text-xl text-slate-400">{t('noJobsFound')}</p>
